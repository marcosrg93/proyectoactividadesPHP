...
(function() {
    ...
    function actualizar(objetoJson) {
        ...
        if (objetoJson.profesores) {
            $('#titulo-pagina').text('Lista Profesores');
            $('#tabladatos').append(getMetaDatosTable(objetoJson.profesores));
            $('.btadd').attr("data-ruta", 'profesor');
        }

        if (objetoJson.actividades) {
            $('#titulo-pagina').text('Lista Actividades');
            $('#tabladatos').append(getMetaDatosTable(objetoJson.actividades));
            $('.map').on('click', cargarVerMapa);
            $('.edit').on('click', cargarEditarMapa);
            $('#btAniadir').on('click', cargarAniadirMapa);
    ...
    function infoUsuario(objetoJson) {
        ...
        $('.btPerfil').addClass('edit');
        $('.btPerfil').attr('data-ruta', 'profesor');
        $('.btPerfil').attr('data-id-modificar', objetoJson.profesor.idProfesor);
        $('.btPerfil').attr('data-target', '#myModalprofesor');
        $('.btPerfil').attr('data-toggle', 'modal');
        //<a  class="btn btn-warning edit center-block" data-toggle="modal" data-target="#myModal' + data + '" data-id-modificar="' + dato[id] + '" data-ruta="' + data + '" data-longitud="' + dato[longitud] + '" data-latitud="' + dato[latitud] + '"><i class="fa fa-pencil-square-o" aria-hidden="true"></i> Modificar</a>
    }
        ...
        for (var i = 0; i < datos.length; i++) {
            ...
            for (var j = 0; j < propiedades.length; j++) {
                ...
                if (propiedades[j] == 'Borrar') {
                    ...
                    table += '<a  class="btn btn-warning edit center-block" data-toggle="modal" data-target="#myModal' + data + '" data-id-modificar="' + dato[id] + '" data-ruta="' + data + '" data-longitud="' + dato[longitud] + '" data-latitud="' + dato[latitud] + '"><i class="fa fa-pencil-square-o" aria-hidden="true"></i> Modificar</a>';
                }
                    ...
                    table += '<a class="btn btn-success map center-block" data-toggle="modal" data-target="#myModalMap" data-longitud="' + dato[longitud] + '" data-latitud="' + dato[latitud] + '"><i class="fa fa-map-marker" aria-hidden="true"></i> Ver mapa</a>' /*'<iframe src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d3179.635215416497!2d-3.593647084271136!3d37.161372479875766!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0xd71fca6107464e5%3A0x5cc395f96e47c0d9!2sInstituto+de+Educaci%C3%B3n+Secundaria+Zaid%C3%ADn+Vergeles!5e0!3m2!1ses!2ses!4v1487583600048" width="350" height="250" frameborder="0" style="border:0" allowfullscreen></iframe>'*/ ;
                }
                else {
        ...
        $('#btAniadir').on('click', function() {
            ...
            switch (tipo) {
                case 'actividades':
                    ...
                    selectProfesores($('#addListaProfesoresAct'), idUser);
    ...
    function addActividades() {
        var formulario=document.getElementById('addFormAct');
        var forData=new FormData(formulario);
        var archivo = document.getElementById("addFileAct");
        forData.append('imagen', archivo.files[0]);
        $.ajax({
            url: 'index.php',
            data: forData,
            type: "POST",
            ...
            if (objetoJson.r != 0) {
                ...
                getInfoActividad();
                page = objetoJson.page;
                $('#myModalInsertactividades').modal('hide');
            }
    ...
    function modificarActividades(button) {
        var formulario=document.getElementById('edFormAct');
        var forData=new FormData(formulario);
        var archivo = document.getElementById("edFileAct");
        forData.append('imagen', archivo.files[0]);
        $.ajax({
            url: 'index.php',
            data: forData,
